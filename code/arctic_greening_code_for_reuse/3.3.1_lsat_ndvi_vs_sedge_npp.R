...
# # plot mean ndvi time series
# par(new=T)
# plot(ndvi.3yr.med ~ year, bio.ts.mc.smry.dt, type='l', xlim=c(1990,2017), ylim=c(0.40,0.65), yaxt='n', xaxt='n', ylab='', xlab='', col=ndvi.col, lwd=2, pch=16)
# axis(4, seq(0.45,0.65,0.05), las=2, cex.axis = my.cex)
# mtext(ndvi.lab, 4, line = 5, cex = my.cex, col=ndvi.col)
# shade.confidence.interval(bio.ts.mc.smry.dt$year, lowerCI = bio.ts.mc.smry.dt$ndvi.3yr.med.q025, upperCI = bio.ts.mc.smry.dt$ndvi.3yr.med.q975, band.col = ndvi.ci.col)
# 
# # SCATTER PLOT
# plotCI(bio.ts.mc.smry.dt$npp.gm2yr.med, bio.ts.mc.smry.dt$ndvi.3yr.med, li = bio.ts.mc.smry.dt$ndvi.3yr.med.q025, ui = bio.ts.mc.smry.dt$ndvi.3yr.med.q975, , err='y', xaxt='n', yaxt='n', 
#        xlim=c(0, 75), ylim=c(0.40,0.65),xlab = '', ylab='',cex = axis.cex, pch=16, scol=adjustcolor('grey50', alpha.f = 0.5))
# plotCI(bio.ts.mc.smry.dt$npp.gm2yr.med, bio.ts.mc.smry.dt$ndvi.3yr.med, li = bio.ts.mc.smry.dt$npp.gm2yr.med.q025, ui = bio.ts.mc.smry.dt$npp.gm2yr.med.q975, err='x', xaxt='n', yaxt='n', 
#        xlim=c(0,75), ylim=c(0.40,0.65),xlab = '', ylab='',cex = axis.cex, pch=16, scol=adjustcolor('grey50', alpha.f = 0.5), add=T)
# axis(1, seq(0,80,20), cex.axis=my.cex)
# axis(2, at = seq(0.45, 0.6,0.05), las=2, cex.axis=my.cex, labels = T)
# mtext(side = 1, line = 3.0, cex = pch.cex, npp.lab, col=npp.col)
# # mtext(side = 2, line = 4.0, cex = pch.cex, ndvi.3yr.lab)
# text(55, 0.44, r.lab, cex = my.cex)
# text(8, 0.565, '(b)', cex = 2, font=2)
