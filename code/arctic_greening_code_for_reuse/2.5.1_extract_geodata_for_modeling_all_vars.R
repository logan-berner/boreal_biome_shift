...
# EXTRACT TERRA CLIMATE VARIABLES =====================================================

tclim.meta.df <- data.frame(var=c('def','pdsi','ppt','soil','srad','tmax','tmin','vpd'),
                            scaler=c(1,0.01,1,1,1,0.01,0.01,0.01),
                            units = c('mm','unitless','mm','mm','Wm2','C','C','kPa'),
                            fun = c('sum','min','sum','min','mean','max','min','max'))
...
# # EXTRACT WORLDCLIM BIOCLIMATE VARIABLES =====================================================
# 
# bioclim.files <- data.frame(file = list.files('data/gis_data/climate/worldclim/', full.names = T),
#                             name = list.files('data/gis_data/climate/worldclim/', full.names = F))
# bioclim.files$name <- gsub('_1km_laea.tif', '', gsub('arctic_worldclim_bio_','bioclim_', bioclim.files$name))
# 
# # extract bioclim
# for (i in 1:nrow(bioclim.files)){
#   r <- raster(as.character(bioclim.files$file[i]))
#   site.dt[, bioclim.files$name[i] := raster::extract(r, pts.laea)]
#   print(i/nrow(bioclim.files))
# }
...
files.to.drop <- c(1:6,8,11,13,15,16,18,19,20,21)
landcov.files <- landcov.files[-files.to.drop, ]
...
landcov.files$name <- gsub('-coverfraction','.covfrac', landcov.files$name)
landcov.files$name <- gsub('forest-type','forest.type', landcov.files$name)
landcov.files$name <- gsub('discrete-classification_EPSG-4326_reproj_LAEA_clip_arctic.tif','landcov', landcov.files$name)

# extract land cover
for (i in 1:nrow(landcov.files)){
  r <- raster(as.character(landcov.files$file[i]))
  site.dt[, landcov.files$name[i] := raster::extract(r, pts.laea)]
  
  # deal with cover fraction of zero being set as NA
  if (any(grep('covfrac', landcov.files$name[i]) == 1) == T){ # convoluted...
    site.dt[is.na(get(landcov.files$name[i])), landcov.files$name[i] := 0]  
  }
  
  # progress
  rm(r)
  print(i/nrow(landcov.files))
}
...
# replace forest type code with class name 
site.dt <- fortype.key[site.dt, on = 'forest.type']
site.dt <- site.dt[, forest.type := NULL]
site.dt <- site.dt[is.na(forest.class), forest.class := 'nonforest']


# WRITE OUT CSV WITH ALL THE DATA ==============================================================================================================
mkdirs('output/lsat_site_trend_modeling')
fwrite(site.dt, 'output/lsat_site_trend_modeling/tundra_site_biogeoclim_ALL.csv')


# DROP SOME OF THE VARIABLES AND WRITE OUT AGAIN ===============================================================================================
lc.vars <- c('forest.class','bare.covfrac','grass.covfrac','moss.covfrac','shrub.covfrac','tree.covfrac')
site.dt <- site.dt[, (lc.vars) := NULL]
drop.cols <- names(site.dt)[c(4:37,40:67)]
site.dt <- site.dt[, (drop.cols) := NULL]

names(site.dt)
fwrite(site.dt, 'output/lsat_site_trend_modeling/tundra_site_biogeoclim.csv')
# site.dt <- fread('output/lsat_site_trend_modeling/tundra_site_biogeoclim.csv')

# END SCRIPT ======================================================================================================
print("All done!!")

# bioclim.nums <- paste0('0',1:19)
# bioclim.nums <- substr(bioclim.nums, nchar(bioclim.nums)-1, nchar(bioclim.nums))
# bioclim.vars <- paste0('bioclim_',bioclim.nums)
# site.dt <- site.dt[, (bioclim.vars) := NULL]
# site.dt[, c('aspect','dem','tpi','tri','slope') := NULL]
