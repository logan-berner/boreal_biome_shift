...
# # monthly fluxes
# tundra.gpp.monthly <- rbind(data.frame(fluxnet.gpp.monthly), data.frame(aon.gpp.monthly))
# tundra.gpp.monthly <- tundra.gpp.monthly %>% filter(gpp.gCm2mon != -9999)
# write.table(tundra.gpp.monthly, 'field_data/tundra_flux_towers/tundra_flux_tower_gpp_monthly_20190122.csv', sep = ',', row.names = F, col.names = T)

# head(tundra.gpp.monthly)
# tundra.gpp.yrly$month <- 13
# setnames(tundra.gpp.yrly, 'gpp.gCm2Yr','gpp.gCm2')
# setnames(tundra.gpp.monthly, 'gpp.gCm2mon','gpp.gCm2')
# tundra.gpp.dt <- rbind(tundra.gpp.monthly, tundra.gpp.yrly)
