...
lsat_pheno = function(dt, vi, window.yrs=15, window.min.obs=30, spar=0.7, spl.fit.outfile){
  # LOAD LIBRARIES
  require(data.table)
  ...
  for (i in 1:n.focal.yrs){
    ...
    spline.fits.dt <- spline.fits.dt[, pos.doy.avg := doy[which.max(spl.fit)], by = 'site'] # calculate typcial day of peak greenness
...
lsat_pheno_max = function(dt, vi, min.frac.of.max = 0.75){
  ...
  dt.smry <- dt[,.(latitude = first(latitude), longitude = first(longitude), n.obs = .N, 
                   ...
                   vi.max.pred = mean(vi.max.pred),
                   vi.max.pred.min = min(vi.max.pred), vi.max.pred.max = max(vi.max.pred),
                   pos.doy.avg=mean(pos.doy.avg)), 
                by = c('site','year')]
