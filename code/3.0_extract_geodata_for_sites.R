...
treecov.r <- raster('data/gis_data/landcov/modis_boreal_treecov_median_2000to2017_laea_300m.tif')
...
clim.trend.files.df$col.name.int <- gsub('to2017','',gsub('trend.','', clim.trend.files.df$col.name))

# SPATIALIZE SITES ===========================================================================
site.dt <- lsat[, .(lat = mean(latitude, na.rm=T), lon = mean(longitude, na.rm=T)), by = site]
pts.wgs84 <- SpatialPoints(coords = site.dt[,c(3,2)], proj4string = wgs84)
pts.laea <- spTransform(pts.wgs84, CRSobj = laea)

## double check alignment
plot(domain.shp, col = 'gray')
points(pts.laea, col = 'orange', pch = '*', cex = 0.5)
...
for (j in 1:nrow(clim.norm.files.df)){
  ...
  data.table(clim := raster::extract(r, site.wgs.pts))
}
